...
for b in [0.3,0.4,0.5,0.6]:
    for p in [0.980,0.985,0.990,0.995]:
        ...
        commands.write('start python main.py ../config/config'+str(idx)+'\n')
        fout=open('../config/config'+str(idx),'w')
        ...
        fout.write('RESULT_DIR =../output/output'+str(b)+'_'+str(p)+'\n')
        fout.write('CALCULATE_SIMILARITY=1\n')
        fout.write('CALCULATE_CLIQUES=1\n')
        fout.write('DISTANCE_DIR=../output\n')
        fout.close()